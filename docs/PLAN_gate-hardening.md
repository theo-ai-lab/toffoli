...
guard — produced ten findings. The headline (the gate could not fail for `d6d7472`) is
fixed in `699f6fb`. Four remain, each reproduced by a concrete mutation. They are
...
- **Each task must be verified by breaking it.** The lesson of `699f6fb` is that a check
...
- [x] **Task 4 — Gate `fsReal.irreversibleUntouched` (D8). DONE 2026-08-02.** `XS`
  ...
  **in-memory** world's restraint. A real adapter that clobbered the dropped table or the
  outbox passes — which is exactly what `d6d7472`'s reused ids did to `outbox/op6.txt`.
...
- **`SqlWorld` is not in the gate at all (D7).** It had the identical id-allocation
  defect (`c121a2e`). Bringing it in is a larger piece of work than any task here and
