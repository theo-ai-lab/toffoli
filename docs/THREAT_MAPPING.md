...
| **ASI03** | Identity & Privilege Abuse | The recovery-context signals (`recoverable`, `externalized`, …) are SAFE-direction and, by default, **believed as given** — a forged `recoverable: true` on a hard delete yields REVERSIBLE at confidence 1.0, which the default policy auto-executes (tested: `lib/mcp/server.test.ts`, "DOCUMENTED BLAST RADIUS"). The attestation layer (`attest.ts` → `sanitizeWithAttestations`) closes that when the caller **turns it on**: pass `attest` to `toffoli.classify` / `toffoli.recover` and every unattested safe-direction signal is stripped before classification, so an impersonating agent's "it's recoverable" fails safe to IRREVERSIBLE. It is **opt-in, not ambient** — an integration that never passes `attest` gets no protection from this row. |
