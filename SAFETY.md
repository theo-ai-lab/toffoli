...
| 11 | **Attestation-gated restore** (`engine/attest.ts`; refuse + escalate on invalid) | "Restore *to* this state" requires a verifiable Ed25519-attested recovery context; an unverifiable baseline is refused and escalated, never restored. | Documented "remediation reverses an intended change" drift class; RATS/RFC 9334 framing. Independence is a disclosed **non-theorem** (see THEORY.md). |
